...
{        
    ...
    private static int _categoriesId;
    ...
    static void Main(string[] args)
    {
        ...
        var inFilePath = Path.IsPathRooted(_inFile) ? _inFile : String.Format("{0}\\{1}", appDir, _inFile);

        //Console.WriteLine(inFilePath);
        if (!File.Exists(inFilePath))
        {
            Console.WriteLine($"map file {inFilePath} not found");
            return;
        }

        _outFile = args.Length > 1 ? args[1] : $"{Path.GetFileNameWithoutExtension(_inFile)}.xdf";
        
        var outFilePath = $"{appDir}\\{_outFile}";
        ...
        BeginXdf(xdfFile);
        categories.Add("Оси");
        _categoriesId = categories.Count - 1;
        WriteAxis(xdfFile);
    ...
    private static void WriteSection(XmlWriter xdfFile, IniSection sectionData)
    {
        ...
        {
            categories.Add(name);
            _categoriesId = categories.IndexOf(name);
        }
    ...
    {
        GetElemParams(sectionData, out var elemFlag, out var elemSize);
    ...
    private static void Write2DTable(XmlWriter xdfFile, IniSection sectionData)
    {
    ...
    private static void WriteTarirovkaDMRV(IniSection sectionData, XmlWriter xdfFile)
    {
        ...
        var count = Convert.ToInt32(sectionData.GetValue("PixelX"));
    ...
    private static void WriteYAxis(IniSection sectionData, XmlWriter xdfFile)
    {
        ...
        var count = Convert.ToInt32(sectionData.GetValue("PixelY"));
        xdfFile.WriteElementString("indexcount", count.ToString());
        ...
        {
            var firstX = ReadDouble(sectionData, "FirstY", 0);
            var lastX = ReadDouble(sectionData, "LastY", 255);
            var delta = (int)Math.Ceiling((lastX - firstX) / count);
    ...
    private static void WriteXAxis(IniSection sectionData, XmlWriter xdfFile)
    {
        ...
        {                                               
            var firstX = ReadDouble(sectionData, "FirstX", 0);
            var lastX = ReadDouble(sectionData, "LastX", 255);
    ...
    {            
        ...
        {
            case "!006-7208":
                labels = [0, 2, 4, 6, 8, 10, 14, 18, 23, 29, 37, 46, 56, 66, 80, 100];
    ...
    private static void WriteAxisLabels(XmlWriter xdfFile, int count, double firstX, float delta)
    {
    ...
    private static void WriteValueEquation(IniSection sectionData, XmlWriter xdfFile)
    {
    ...
    private static void Write1DTable(XmlWriter xdfFile, IniSection sectionData)
    {
        GetElemParams(sectionData, out var elemFlag, out var elemSize);             
    ...
    private static void WriteSectionBit(XmlWriter xdfFile, IniSection sectionData)
    {            
        for (var i = 0; i < 8; i++)
        {
    ...
    private static void WriteCurrentCategory(XmlWriter xdfFile)
    {
        xdfFile.WriteStartElement("CATEGORYMEM");
        xdfFile.WriteAttributeString("index", "0x0");
        xdfFile.WriteAttributeString("category", (_categoriesId + 1).ToString());
        xdfFile.WriteEndElement();
    }

    private static void BeginXdf(XmlWriter xdfFile)
    {
    ...
    private static void EndXdf(XmlWriter xdfFile)
    {
        xdfFile.WriteEndElement();
    }

    private static void WriteHeader(XmlWriter xdfFile)
    {
        xdfFile.WriteStartElement("XDFHEADER");
        xdfFile.WriteElementString("flags", "0x1");
        xdfFile.WriteElementString("fileversion", "02.01h");
        xdfFile.WriteElementString("deftitle", _inFile);
        ...
        for (var i = 0; i < categories.Count; i++)
        {
            ...
            xdfFile.WriteAttributeString("index", $"0x{i.ToString("X2")}");
    ...
    private static void WriteAxis(XmlWriter xdfFile)
    {
        xdfFile.WriteRaw(
    ...
    private static double ReadDouble(IniSection section, string name, double def)
    {
    ...
    private static int ReadInt(IniSection section, string name, int def)
    {
        return section.Contains(name)
    ...
    private static string ReadStr(IniSection section, string name, string def)
    {
