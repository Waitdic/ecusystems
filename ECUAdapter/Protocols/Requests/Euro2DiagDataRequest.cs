...
public sealed class Euro2DiagDataRequest : JRequest, IDiagDataRequest
{
    ...
    public Euro2DiagDataRequest() : base("8210F12101A5")
    {
        testCrc = false;
    ...
    private void ParseValues()
    {
        ...
        DiagData.ErrorStatus = 
            (uint)((replyBuffer[valueOffset + 7] << 24) 
                   + (replyBuffer[valueOffset + 6] << 16) 
                   + (replyBuffer[valueOffset + 5] << 8) 
                   + replyBuffer[valueOffset + 4]);            
    }
