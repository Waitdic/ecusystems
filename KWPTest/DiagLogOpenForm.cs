...
{
    byte[] icdIndexes = [35, 2, 3, 3, 5, 6, 0];
    byte[] matrixIndexes = [];
    byte[] injIndexes = [];

    readonly List<DiagData> diagData = [];
    ...
    public static DiagData[] LoadDiagLog(IWin32Window owner)
    {
        ...
        return form.ShowDialog(owner) != DialogResult.OK ? [] : form.diagData.ToArray();
    }
    ...
    private static DiagData GetInjDiagData(string item)
    {
