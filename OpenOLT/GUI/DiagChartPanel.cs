...
internal partial class DiagChartPanel : UserControl
{
    ...
    private readonly IDictionary<string, Series> _charts = new Dictionary<string, Series>();
    ...
    public void Prepare(OnlineManager onlineManager)
    {
        ...
        _dataKeeper = onlineManager.dataKeeper;
        _dataKeeper.diagDataList.ListChanged += diagDataList_ListChanged;

        foreach (var valueInfo in _dataKeeper.valueInfos)
            chartsSet.Items.Add(valueInfo.Title);
    }

    private void diagDataList_ListChanged(object sender, ListChangedEventArgs e)
    {
        switch (e.ListChangedType)
        {
            case ListChangedType.ItemAdded:
                _uiContext.Send(delegate { UpdateCharts(); }, null);
                break;
    ...
    {
        var data = _dataKeeper.diagDataList.Skip(FirstPoint).Take(ChartScale).ToArray();
        var chart = _charts[name];
    ...
    {
        var value = _dataKeeper.diagDataList.Last();

        foreach (var item in _charts)
        {
            if (item.Value.Points.Count > ChartScale)
                for (var i = 0; i < item.Value.Points.Count - ChartScale; i++)
                    item.Value.Points.RemoveAt(0);
                
            item.Value.Points.AddXY(_properties["_Time"].GetValue(value, null),
                _properties[item.Key].GetValue(value, null));
    ...
    private void AddChart(ValueInfo info)
    {
        ...
        title = new Title
        {
            ...
            ForeColor = Color.FromArgb(200, _palette[_charts.Count % _palette.Length])
        };
        diagChart.Titles.Add(title); 
    ...
    private void openLogAction_Execute(object sender, EventArgs e)
    {
        ...
        _dataKeeper.LoadFromFile(openFileDialog.FileName);
    }

    private void openChartsAction_Execute(object sender, EventArgs e)
    {
        var newCharts = _dataKeeper.OpenCharts();
    ...
    {
        if (ChartScale > _dataKeeper.diagDataList.Count) return;
        var factor = (ModifierKeys & Keys.Control) == Keys.Control ? 1.5 : 1.1;
        ChartScale = (int)Math.Ceiling(ChartScale * factor);
        if (_onlineManager != null && _onlineManager.OltProtocol.Connected) return;
        LoadCharts();
    }
    ...
    {
        if (FirstPoint >= _dataKeeper.diagDataList.Count - ChartScale) return;
    ...
    private void OfflineAction_Update(object sender, EventArgs e)
    {
        ...
        action.Enabled = _onlineManager != null && !_onlineManager.OltProtocol.Connected;
    }
    ...
    {
        var valueInfo = _dataKeeper.valueInfos[e.Index];
