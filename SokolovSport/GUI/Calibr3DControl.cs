...
internal partial class Calibr3DControl : UserControl
{
    ...
    private void Calibr1DChartOnMouseMove(object sender, MouseEventArgs mouseEventArgs)
    {
        ...
        // get last mouse location within control //'
        var pt = calibr1DChart.PeUserInterface.Cursor.LastMouseMove;
        var pX = pt.X;
        var pY = pt.Y;
    ...
    private void FillPalette()
    {
        paletteScale = 1;
        paletteFast.Clear();
        if (calibrItem.Table == null) return;
        
        CalcMinMax(out var min, out var max);
    ...
    private void Prepare3DChart()
    {
        ...
        calibr3DChart.PeString.ZAxisLabel = calibrItem.AxisYCalibrItem == null
            ? string.Empty
            : $"{calibrItem.AxisYCalibrItem.Name}, {calibrItem.AxisYCalibrItem.Unit}";
        calibr3DChart.PeString.XAxisLabel = calibrItem.AxisXCalibrItem == null
            ? string.Empty
            : $"{calibrItem.AxisXCalibrItem.Name}, {calibrItem.AxisXCalibrItem.Unit}";
    ...
    {
        var colCount = calibrItem.Table?.ColCount ?? 1;
        var rowCount = calibrItem.Table?.RowCount ?? 1;
    ...
    private void Init1DChart(int colCount, int rowCount)
    {
        ...
        calibr1DChart.PeString.YAxisLabel = $"{calibrItem.Name}, {calibrItem.Unit}";
        calibr1DChart.PeString.XAxisLabel = axisCalibr == null ? string.Empty : $"{axisCalibr.Name}, {axisCalibr.Unit}";
        ...
        {
            calibr1DChart.PeData.X[0, i] = axisCalibr?.CalcValueByIndex(i, pointCount) ?? i;
            calibr1DChart.PeData.Y[0, i] = calibrItem.Table?.GetValue(i) ?? calibrItem.Value;
        }
    ...
    private void InitTable()
    {
        ...
        if (colCalibr != null)
            for (var i = 1; i <= calibrItem.Table.ColCount; i++)
            {
                ...
                calibrGrid.Cols[i].Format = "f" + calibrItem.ItemInfo.Precision;
        ...
        for (var i = 1; i <= calibrItem.Table.RowCount; i++)
        for (var j = 1; j <= calibrItem.Table.ColCount; j++)
    ...
    private void calibrGrid_AfterEdit(object sender, C1.Win.C1FlexGrid.RowColEventArgs e)
    {
        ...
        float value;
        if (!float.TryParse(calibrGrid[e.Row, e.Col].ToString(), out value)) return;
    ...
    {            
        ...
        try
        {
            if (!float.TryParse(e.Text, out var value)) return;
