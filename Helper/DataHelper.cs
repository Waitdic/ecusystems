...
public static class DataHelper
{
    public static bool Compare(byte[] b1, byte[] b2)
        => memcmp(b1, b2, new IntPtr(b1.Length)) == IntPtr.Zero;
    ...
    {
        if (buffer == null || count < 1 || count > buffer.Length - index) 
            return string.Empty;
    ...
    {            
        if (buffer == null || count < 1 || count > buffer.Length - index) return [];
    ...
    public static string ByteArrayToStr(byte[] buffer, int count) 
        => ByteArrayToStr(buffer, 0, count);

    public static string ByteArrayToStr(byte[] buffer) 
        => ByteArrayToStr(buffer, buffer?.Length ?? 0);

    public static byte Rl(byte source) 
        => (byte)((byte)(source << 1) | (byte)(source >> 7));
