...
class Program
{
    ...
    static void Main(string[] args)
    {
        ...
        try
        {
            ...
            {                    
                ...
                Request(serialPort, StartCommunication, out var response);
    ...
    private static string CalcSerialNumber(byte[] response)
    {
        ...
        if (response.Length < 13) 
            return "Error serial number response " + str;
        ...
        out byte[] response)
    {
        ...
        if (!success) 
            throw new Exception($"Error response: {BytesToStr(response, response.Length)}");
    }
    ...
    {
        return BytesToStr(buffer, buffer?.Length ?? 0);
    }
